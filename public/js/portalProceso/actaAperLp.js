...
function cargarCamposGenerales(datos) {
    ...
    if (fechaFallo) {
        fechaFallo.innerText = primerRegistro.fecha_fallo ?? '';
    }
...
function cargarTextoParticipantes(datos) {
    ...
    texto += `; Y CON FUNDAMENTO EN LO DISPUESTO EN LOS ARTÍCULOS 28 FRACCIÓN I, 35 FRACCIÓN I Y 36, DE LA LEY DE CONTRATACIÓN PÚBLICA DEL ESTADO DE MÉXICO Y MUNICIPIOS; 55 FRACCIÓN II, 67 FRACCIÓN V, 68 FRACCIÓN I, 82, 83, 84, 85 Y 86 FRACCIÓN I; DE SU RESPECTIVO REGLAMENTO, SE LLEVÓ A CABO EL ACTO EL ACTO DE PRESENTACIÓN Y APERTURA DE PROPUESTAS DEL PROCEDIMIENTO DE LICITACIÓN PÚBLICA NACIONAL PRESENCIAL LPNP${datos[0]?.no_licitacion ?? ''}, PARA LA "${datos[0]?.nombre_estudio ?? ''}"`;
