...
// ==============================================
function formatearFechaAnexo($fechaString) {
...
function formatearHoraAnexo($fechaString) {
    if (empty($fechaString)) return '';
    $fecha = new DateTime($fechaString);
    return $fecha->format('H:i');
}

function formatearFechaHoraCompletaAnexo($fechaString) {
    if (empty($fechaString)) return '';
    $fecha = new DateTime($fechaString);
    $meses = ['ENERO','FEBRERO','MARZO','ABRIL','MAYO','JUNIO','JULIO','AGOSTO','SEPTIEMBRE','OCTUBRE','NOVIEMBRE','DICIEMBRE'];
    ...
    $horasTexto = numeroATextoAnexo((int)$horas);
    $minutosTexto = numeroATextoAnexo((int)$minutos);
    
    return $horasTexto . ' HORAS CON ' . $minutosTexto . ' MINUTOS DEL DÍA ' . $fecha->format('d') . ' DE ' . $meses[$fecha->format('n')-1] . ' DE ' . $fecha->format('Y');
}

function numeroATextoAnexo($numero) {
    ...
    <div class="contenido-hoja">
        ...
        <div class="texto">
            ...
            <span class="campo campo-corto">
                <?= isset($data[0]['created_at']) ? formatearHoraAnexo($data[0]['created_at']) : '' ?>
            </span>
            HORAS DEL DÍA
            <span class="campo campo-chico">
                <?= isset($data[0]['created_at']) ? formatearFechaAnexo($data[0]['created_at']) : '' ?>
            </span>,
        ...
        <div class="texto">
            <?php if (!empty($data)): ?>
                ...
                <?php foreach ($data as $index => $item): ?>
                    ...
                    <?php if (isset($item['tipo_persona']) && !empty($item['tipo_persona'])): ?>
                        , EN SU CARÁCTER DE <span class="campo campo-mediano"><?= esc($item['tipo_persona']) ?></span>
                    <?php endif; ?>
                    <?php if (isset($item['num_credencial_representante']) && !empty($item['num_credencial_representante'])): ?>
                        , MISMO QUE SE IDENTIFICÓ CON CREDENCIAL PARA VOTAR CON NÚMERO DE FOLIO <span class="campo campo-mediano"><?= esc($item['num_credencial_representante']) ?></span> EMITIDA POR EL INSTITUTO NACIONAL ELECTORAL
                    <?php endif; ?>
                <?php endforeach; ?>
...
<div class="hoja">
    <div class="contenido-hoja">
        ...
        <div class="texto">
            <strong>VII.</strong> <strong>EN DESAHOGO DEL SÉPTIMO PUNTO DEL ORDEN DEL DÍA.</strong> – EL SERVIDOR PÚBLICO DESIGNADO, ANUNCIÓ QUE SE ACEPTARON PARA SU <strong>ANÁLISIS Y EVALUACIÓN</strong> DE LOS INTEGRANTES DEL COMITÉ DE ADQUISICIONES Y SERVICIOS, ASÍ COMO DEL ÁREA USUARIA, LA PROPUESTA TÉCNICA Y DOCUMENTACIÓN COMPLEMENTARIA DE LOS LICITANTES QUE PRESENTARON SU PROPUESTA, EN VIRTUD DE QUE CUMPLIERON CUANTITATIVAMENTE, CON LOS REQUISITOS ESTABLECIDOS EN LAS BASES EMITIDAS, MISMOS QUE SE CITA A CONTINUACIÓN:
        </div>
        ...
        <div class="firma-contenedor-unica">
            <div class="firma-unica">
                ...
                <div class="linea-firma">
                    <span><?= isset($data[0]['coordinador']) ? esc($data[0]['coordinador']) : 'C. PAULO SERGIO HERNÁNDEZ CUADRIELLO' ?></span>
                </div>
            </div>
        </div>
